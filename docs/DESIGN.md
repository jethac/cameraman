...
- `CameramanStoryboard`: `show_image`, `image: Texture2D`, `aspect {BEST_FIT, CROP_IMAGE_TO_FIT, STRETCH_TO_FIT}`, `alpha`, `center`, `rotation`, `scale`, `mute_camera`, `split_view` — TextureRect on a CanvasLayer created lazily, visible only while the camera is live.
