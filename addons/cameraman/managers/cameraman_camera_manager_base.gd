...
## Virtual camera base that selects and blends child virtual cameras.
extends CameramanVirtualCameraBase

## Blend used when a child manager does not provide a specific rule.
@export var default_blend: CameramanBlendDefinition
## Named blend rules used for child-camera transitions.
...
## Returns direct child virtual cameras eligible for manager selection.
...
## Returns true when camera is the manager's active child or blend endpoint.
func is_live_child(camera: CameramanVirtualCameraBase) -> bool:
	return _manager.is_live(camera)

## Selects custom or default blending for child-camera transitions.
